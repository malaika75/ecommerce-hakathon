...
//       {showPopup && (
//         <div className="popup fixed top-0 left-1/2 transform -translate-x-1/2 mt-4 bg-green-500 text-white p-4 rounded shadow-lg z-10">
//           <p>Great Choice! Its now in your cart! 🛒</p>
//         </div>
//       )}

//       {wishlistPopup && (
//         <div className="popup fixed top-0 left-1/2 transform -translate-x-1/2 mt-4 bg-yellow-700 text-white p-4 rounded shadow-lg z-10">
...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (userId !== "guest" && guestCart && !storedCart) {
      // migrate guest cart to user cart (safe delay before removing guest cart)
      localStorage.setItem(cartKey, guestCart);
      setCartItems(JSON.parse(guestCart));
      setTimeout(() => {
        localStorage.removeItem("cartItems_guest");
