...
async function uploadProduct(product, index) {
  try {
    // Check if product already exists based on title (or unique _id)
    ...
    if (existingProduct) {
      ...
      return; // Skip the product if it already exists
    }
...
async function importProducts() {
  try {
    ...
    for (const product of products) {
      await uploadProduct(product, products.indexOf(product));
    }
  } catch (error) {
